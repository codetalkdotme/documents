...
public class ParamCheckerImpl implements IParamChecker {
	...
	// src type参数
	static Param PARAM_SRCTYPE = new StringEnumParam("src_type", true, 
			new String[] {Constants.SOURCE_TYPE_HECADRE, Constants.SOURCE_TYPE_INHABITANT, Constants.SOURCE_TYPE_DOCTOR});
	...
	static Param PARAM_DEVID_OPTIONAL = new StringParam("dev_id", false, true);
	// SMS type
	...
	// fdc service item 
//	static Param PARAM_SERVICE_ITEM = new IntListEnumParam("service_item", false, new Integer[] {
...
//	}, true);
	...
	static Param OTHER_HOSPITAL_NAME = new StringParam("other_hospital_name", false , true);
	
	static Param HOSPITAL_ID_LIST = new LongListParam("hospital_id_list", false);
	...
	// 病情说明
	static Param NOTE = new StringParam("note", false , true);
	...
	// 新增的参加活动的居民用户ID列表
	static Param USER_ID_ADD_LIST = new LongListParam("user_id_add_list", false);
	
	// 要获取状态的用户ID列表
	static Param USER_ID_GET_LIST = new LongListParam("user_id_get_list", true);
	...
	static {
		// 检测前端版本
		URI_PARAM_MAPPING.put("/hca/api/update/checkver", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_DEVID_REQUIRED,
				new StringParam("ver", true), //  "ver":"xx" //前端版本号
				new LongParam("ver_code", false, 1L)});
		
		// 获取用户基本信息
		URI_PARAM_MAPPING.put("/hca/api/auth/getuserbasicinfo", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_DEVID_REQUIRED});
		
		// 获取短信验证码
		URI_PARAM_MAPPING.put("/hca/api/auth/getsmscode", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_SMSTYPE, PARAM_MOBILE_REQUIRED});
		
		// 添加用户
		URI_PARAM_MAPPING.put("/hca/api/auth/adduser", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, 
				...
				new StringParam("person_id_pic_url", false)});
		
		// 用户注册
		URI_PARAM_MAPPING.put("/hca/api/auth/registeruser", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, 
		...
		// 认证登录
		URI_PARAM_MAPPING.put("/hca/api/auth/userlogin", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, 
		...
		// 获取用户信息 
		URI_PARAM_MAPPING.put("/hca/api/auth/getuserinfo", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, 
				new LongListParam("user_id_get", false),
				new StringListParam("person_id_get", false),
				new StringListParam("name_get", false)});
		
		// 修改用户信息
		URI_PARAM_MAPPING.put("/hca/api/auth/setuserinfo", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				...
				PARAM_SEX_OPTIONAL, 
				new StringParam("head_pic_url", false),
				new StringParam("person_id_pic_url", false), // person_id_pic_url@0707
				PARAM_DEVID_OPTIONAL});
...
//			  "passwd_str":"xx" //密码密文串
//			}
		URI_PARAM_MAPPING.put("/hca/api/auth/resetuserpasswd", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, 
		...
		// 修改用户手机号码
		URI_PARAM_MAPPING.put("/hca/api/auth/setusermobile", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new RegexParam("new_mobile", true, "1\\d{10}"),
				new StringParam("sms_code")});
		
		// 服务登录
		URI_PARAM_MAPPING.put("/hca/api/business/login", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new StringParam("svc_ticket_str")});
		
		// 获取下级行政区列表
		URI_PARAM_MAPPING.put("/hca/api/business/getarealist", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new LongParam("area_id_higher", false, 1L)});
		
		// 意见反馈
		URI_PARAM_MAPPING.put("/hca/api/business/comment", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new StringParam("comment")});
		
		// 获取内部推送消息
		URI_PARAM_MAPPING.put("/hca/api/business/getpushmsg", PARAM_ARR_COMMON);
		
		// 获取任务计划
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/gettasksum", PARAM_ARR_COMMON);

		// 添加任务计划
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/addtask", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new LongListParam("add_visit_tenement_id", false)});
		
		// 获取通知消息
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getnotice", PARAM_ARR_COMMON);
		
		// 获取备忘列表
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getmemolist", PARAM_ARR_COMMON_PAGE);

		// 修改备忘
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/modmemo", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new LongParam("memo_id", false, 1L),
				new StringParam("title", false), new StringParam("content", false)});
		
		// 获取家庭医生签约列表
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getfdcontractlist", PARAM_ARR_COMMON_PAGE);
		
		// 获取家庭医生签约信息
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getfdcontract", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new LongParam("fdcontract_id", false, 1L), new LongParam("tenement_id", false, 1L)});
		
		// 修改家庭医生签约信息
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/modfdcontract", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				new LongParam("fdcontract_id", true, 1L), 
				new StringParam("social_security_card_no", false),
				new IntEnumParam("fdcontract_state", true, 
						new Integer[]{Constants.FDC_STATE_NOT_APPLIED, Constants.FDC_STATE_APPLIED, Constants.FDC_STATE_CONFIRMED, Constants.FDC_STATE_CANCELED}),
				new LongParam("begin_time", true, 0L),
				new LongParam("end_time", true, 0L), 
				new StringParam("fd_sign_pic_url", false),
				new StringParam("user_sign_pic_url", false)
				});
		
		// 获取所辖居民
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getinhabitant", PARAM_ARR_COMMON_PAGE);
		
		// 获取通知消息
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/getnotice", PARAM_ARR_COMMON_PAGE);

		// 回复通知消息
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/replynotice", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR,
				...
				new LongParam("appo_time", false, 1L)});
		
		// 获取对应健教专干
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/gethecadre", PARAM_ARR_COMMON);
		
		// 获取家庭医生签约申请
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/getfdcontractstate", PARAM_ARR_COMMON);

		// 提交家庭医生签约申请
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/signfdcontract", 
				new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR});
		

		//获取交流圈列表
		URI_PARAM_MAPPING.put("/hca/api/business/getcirclelist", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID});
		//获取交流圈留言列表
		URI_PARAM_MAPPING.put("/hca/api/business/getcirclepostlist", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , CIRCLE_ID , PARAM_BEGIN , PARAM_COUNT});
		//发表交流圈留言
		URI_PARAM_MAPPING.put("/hca/api/business/submitcirclepost", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , CIRCLE_ID , POST_TITLE , POST_CONTENT , POST_PIC_URL_LIST});
		//删除交流圈留言
		URI_PARAM_MAPPING.put("/hca/api/business/delcirclepost", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , POST_ID});
		//获取预约挂号列表
		URI_PARAM_MAPPING.put("/hca/api/business/getapporeglist", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , PARAM_BEGIN , PARAM_COUNT});
		//修改预约挂号
		URI_PARAM_MAPPING.put("/hca/api/business/modapporeg", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , APPO_REG_ID , OTHER_HOSPITAL_NAME , HOSPITAL_ID_LIST , DEPT_ID , USER_ID_CLINIC , USER_ID_APPO , 
				NOTE , REMARK_HECADRE , APPO_STATE , APPO_CLINIC_TIME , SURE_APPO_TIME , CLINIC_TIME});
		//获取计划免疫列表
		URI_PARAM_MAPPING.put("/hca/api/business/getappoimmunelist", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , PARAM_BEGIN , PARAM_COUNT});
		//修改计划免疫
		URI_PARAM_MAPPING.put("/hca/api/business/modappoimmune", new Param[] {PARAM_SRCTYPE, PARAM_PFTYPE, PARAM_UID , PLAN_IMMUNE_ID , USER_ID_IMMUNE , USER_ID_APPO , NOTICE_CONTENT , REMARK_HECADRE ,
				APPO_STATE , APPO_IMMUNE_TIME , SURE_APPO_TIME , IMMUNE_TIME});
		//获取活动类型状态
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getactivitystate", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID});
		//获取活动信息
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getactivity", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , ACTIVITY_TYPE});
		//发起活动
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/startactivity", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , ACTIVITY_TYPE , ACTIVITY_TIME , ACTIVITY_PLACE , SUBJECT , USER_ID_TO_NOTICE_LIST});
		//活动签到或登记
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/signinactivity", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , ACTIVITY_ID , USER_SIGN_LIST});
		//进入活动总结
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/enteractivitysum", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , ACTIVITY_ID });
		//修改交流圈
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/modcircle", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , UPDATE_CIRCLE_ID , CIRCLE_NAME , CIRCLE_INTRO , MEMBER_LIST , HEAD_PIC_URL });
		//获取预约挂号医院及常见病种列表
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/getappohospitalillnesslist", new Param[] {PARAM_INHABITANT_SRCTYPE, PARAM_PFTYPE, PARAM_UID });
		//获取预约挂号医师列表
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/getappodoctorlist", new Param[] {PARAM_INHABITANT_SRCTYPE, PARAM_PFTYPE, PARAM_UID , HOSPITAL_ID , ILLNESS_ID});
		//添加参加活动的人员
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/addactivityuser", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , ACTIVITY_ID , USER_ID_ADD_LIST});
		//获取活动中居民的状态
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/getactivityuserstate", new Param[] {PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID , ACTIVITY_ID , USER_ID_GET_LIST});
		
		// 居民端首页文章列表
		URI_PARAM_MAPPING.put("/hca/api/business/inhabitant/gethomearticlelist", new Param[] { PARAM_INHABITANT_SRCTYPE,
				PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, PARAM_BEGIN, PARAM_COUNT });
		// 专家讲堂居民端获取文章列表
		URI_PARAM_MAPPING.put("/hca/web/inhabitant/getarticlelist", new Param[] { PARAM_BEGIN, PARAM_COUNT });
		// 专家讲堂居民端获取文章详情
		URI_PARAM_MAPPING.put("/hca/web/inhabitant/getarticledetail", new Param[] { ARTICLE_ID, PARAM_UID });
		// 居民端文章收藏列表
		URI_PARAM_MAPPING.put("/hca/web/inhabitant/getarticlefavoritlist", new Param[] { PARAM_INHABITANT_SRCTYPE,
				PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, PARAM_BEGIN, PARAM_COUNT });
		// 居民端文章收藏提交
		URI_PARAM_MAPPING.put("/hca/web/inhabitant/submitarticlefavorite", new Param[] { PARAM_INHABITANT_SRCTYPE,
				PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, ARTICLE_ID, PARAM_UID });
		// 居民端文章取消收藏
		URI_PARAM_MAPPING.put("/hca/web/inhabitant/canclearticlefavorite", new Param[] { PARAM_INHABITANT_SRCTYPE,
				PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, ARTICLE_ID, PARAM_UID });
		// 专家讲堂健教端获取文章列表
		URI_PARAM_MAPPING.put("/hca/web/hecadre/getarticlelist", new Param[] { PARAM_BEGIN, PARAM_COUNT });
		// 专家讲堂健教端获取分类下拉
		URI_PARAM_MAPPING.put("/hca/web/hecadre/getarticlecategory", new Param[] {});
		// 专家讲堂健教端发表文章
		URI_PARAM_MAPPING.put("/hca/web/hecadre/submitarticle",
				new Param[] { PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, ARTICLE_TITLE,
						ARTICLE_CATEGORY, ARTICLE_AUTHOR, ARTICLE_SOURCE, ARTICLE_KEYWORDS, ARTICLE_IMG_URL,
						ARTICLE_SUMMARY, ARTICLE_CONTENT, PARAM_UID });
		// 专家讲堂健教端获取修改文章信息
		URI_PARAM_MAPPING.put("/hca/web/hecadre/getarticlemod", new Param[] { ARTICLE_ID });
		// 专家讲堂健教端修改文章
		URI_PARAM_MAPPING.put("/hca/web/hecadre/modarticle",
				new Param[] { PARAM_HECADRE_SRCTYPE, PARAM_PFTYPE, PARAM_UID, PARAM_AUTHSTR, ARTICLE_TITLE,
						ARTICLE_CATEGORY, ARTICLE_AUTHOR, ARTICLE_SOURCE, ARTICLE_KEYWORDS, ARTICLE_IMG_URL,
						ARTICLE_SUMMARY, ARTICLE_CONTENT, PARAM_UID });
		
		// 添加家庭成员
		URI_PARAM_MAPPING.put("/hca/api/business/hecadre/modfamilymember", PARAM_ARR_COMMON);
